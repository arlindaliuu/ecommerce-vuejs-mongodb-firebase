...
<form @submit.prevent="handleEditProduct" enctype="multipart/form-data">
    <h1 class="mx-64 font-bold text-3xl">Edito Produkt</h1>
    <div class="mx-64 border mt-5 p-5 shadow-2xl px-32">
        <div class="form-group grid grid-cols-2 p-5">
            ...
            <input class="border  h-10" id="name" name="name" v-model="product.title" />
        </div>
        <div class="form-group grid grid-cols-2 p-5">
            <label class="" for="description">Përshkrimi</label>
            <textarea class="border" id="description" name="description" v-model="product.description" />
        </div>
        <div class="form-group grid grid-cols-2 p-5">
            <label class="" for="author">Autori</label>
            <input class="border h-10" id="author" name="author" v-model="product.author" />
        </div>
        <div class="form-group grid grid-cols-2 p-5">
        ...
        <div class="form-group grid grid-cols-2 p-5">
            ...
            <div class="flex flex-wrap justify-around">
                <div class="border flex flex-wrap justify-center">
                    ...
                    <input type="checkbox" id="red" value="red" v-model="product.ngjyra">
                  </div>
                  <div class="border flex flex-wrap justify-center ml-2">
                    <label for="Jeshile">Jeshile</label>
                    <input type="checkbox" id="Jeshile" value="Jeshile" v-model="product.ngjyra">
                  </div>
                  <div class="border flex flex-wrap justify-center  ml-2">
                    <label for="Blu">Blu</label>
                    <input type="checkbox" id="Blu" value="Blu" v-model="product.ngjyra">
                  </div>
                  <div class="border flex flex-wrap justify-center  ml-2">
                      <label for="Bardhë">Bardhë</label>
                      <input type="checkbox" id="Bardhë" value="Bardhë" v-model="product.ngjyra" v-if="product.ngjyra ? 'checked' : 'a'">
                    </div>
                    <div class="border flex flex-wrap justify-center  ml-2">
                        <label for="Hiri">Hiri</label>
                        <input type="checkbox" id="Hiri" value="Hiri" v-model="product.ngjyra">
                    </div>
                    <div class="border flex flex-wrap justify-center  ml-2">
                        <label for="Portokalli">Portokalli</label>
                        <input type="checkbox" id="Portokalli" value="Portokalli" v-model="product.ngjyra">
                    </div>
                    <div class="border flex flex-wrap justify-center  ml-2">
                        <label for="E Zezë">E Zezë</label>
                        <input type="checkbox" id="E Zezë" value="E Zezë" v-model="product.ngjyra">
                    </div>
                    <div class="border flex flex-wrap justify-center  ml-2">
                        <label for="Verdhë">Verdhë</label>
                        <input type="checkbox" id="Verdhë" value="Verdhë" v-model="product.ngjyra">
                    </div>
                    <div class="border flex flex-wrap justify-center  ml-2">
                        <label for="Rozë">Rozë</label>
                        <input type="checkbox" id="Rozë" value="Rozë" v-model="product.ngjyra">
                    </div>
                </div>
            </div>
        ...
        <div class="form-group grid grid-cols-2 p-5">
            ...
            <input placeholder="Opsional" class="border h-10" id="rating" type="number" name="rating" v-model="product.rating" />
        </div>
        ...
        <div class="w-full flex justify-center">
            <button class="border px-5 bg-green-900 rounded-lg text-white py-2">Edito Produktin</button>
        </div>
    </div>
</form>
</template>
...
export default{
    ...
    methods:{
        async handleEditProduct(){
            ...
            .then((success) => {
                if (success) {
                    this.$refs.toaster.show(`U editua me sukses!`, "success");
                } else {
                    this.$refs.toasterError.show(`Ndodhi një gabim gjatë editimit të produktit`, "wrong");
                }
                })
                .catch((error) => {
                console.error(error);
